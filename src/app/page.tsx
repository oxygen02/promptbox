...
// 积分消耗计算函数（免费模型 vs 收费模型）
// 收费模型成本约为免费模型的1.6-2倍
const CREDIT_COSTS = {
  text: { analyze: 3, generate: 5 },      // 文字文档
  image: { analyze: 8, generate: 25 },    // 图片视觉
  video: { analyze: 15, generate: 0 },  // 视频解构（不开放生成）
  web: { analyze: 5, generate: 5 },     // 网页设计
};

const CREDIT_COSTS_PAID = {
  text: { analyze: 5, generate: 8 },     // 文字文档
  image: { analyze: 15, generate: 50 },   // 图片视觉
  video: { analyze: 30, generate: 0 },   // 视频解构（不开放生成）
  web: { analyze: 8, generate: 8 },      // 网页设计
};

// 获取积分消耗（默认免费模型，可切换为收费模型）
function getContentTypeCost(contentType: string, action: 'analyze' | 'generate', isPaidModel: boolean = false) {
  const costs = isPaidModel ? CREDIT_COSTS_PAID : CREDIT_COSTS;
  return costs[contentType as keyof typeof costs]?.[action] || 0;
}
      ...
      <div className="relative z-10">
        ...
        <div className="glass-card rounded-2xl p-5 mb-4">
          ...
          <div className="flex gap-4">
            ...
            <div className="w-1/2">
              {/* 积分消耗提示 - 根据内容类型显示 */}
              <div className="text-xs text-slate-500 bg-slate-50 px-2 py-1.5 rounded mb-2 flex items-center gap-1">
                <span className="text-amber-500">🪙</span>
                <span>
                  {language === 'zh' 
                    ? `${t.contentTypes.find(c => c.key === contentType)?.label || '文字文档'}: 分析 ${getContentTypeCost(contentType, 'analyze')} 积分`
                    : `${t.contentTypes.find(c => c.key === contentType)?.label || 'Text'}: Analyze ${getContentTypeCost(contentType, 'analyze')} credits`
                  }
                </span>
              </div>
              <div 
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex items-center gap-2 text-sm">
            <span className="text-amber-500 font-medium">Credit</span>
            <span className="text-slate-700 font-semibold">{credits}</span>
          </div>
          {/* 积分消耗提示 */}
          <div className="text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded">
            {language === 'zh' 
              ? `本次分析将扣除 ${getContentTypeCost(contentType, 'analyze')} 积分`
              : `This analysis will cost ${getContentTypeCost(contentType, 'analyze')} credits`
            }
          </div>
        </div>
        ...
        {contentType !== 'video' && (
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="text-xs text-slate-500 bg-slate-50 px-2 py-1 rounded">
              {language === 'zh' 
                ? `生成将扣除 ${getContentTypeCost(contentType, 'generate')} 积分`
                : `Generate will cost ${getContentTypeCost(contentType, 'generate')} credits`
              }
            </div>
            {/* 图片数量选择 - 只在图片视觉类型显示 */}
            {contentType === 'image' && (
