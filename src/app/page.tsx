      ...
      <div className="relative z-10">
          ...
          {[0, 1, 2].map((index) => { const model = cardModels[index]; const prompt = cardPrompts[index]; const modelInfo = MODELS.find((m) => m.key === model); return (
            <div key={index} className="glass-card rounded-xl p-4">
              <div className="relative" ref={dropdownRef}>
                ...
                {openDropdown === index && (<div className="dropdown-menu"><div className={cn("dropdown-item", !model && "active")} onClick={() => handleCardModelSelect(index, null)}>— 未选中 —</div>{MODELS.map((m) => (<div key={m.key} className={cn("dropdown-item", model === m.key && "active")} onClick={() => handleCardModelSelect(index, m.key)}>{m.name} ({m.region})</div>))}</div>)}
              </div>
        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="relative" ref={dropdownRef}>
            ...
            {openGenDropdown && (<div className="dropdown-menu">{MODELS.map((m) => (<div key={m.key} className={cn("dropdown-item", selectedGenModel === m.key && "active")} onClick={() => { setSelectedGenModel(m.key); setOpenGenDropdown(false); }}>{m.name}</div>))}</div>)}
          </div>
          <div className="flex items-center gap-2 text-sm"><span className="text-amber-500 font-medium">Credit</span><span className="text-slate-700 font-semibold">{credits}</span></div>
        </div>
