...
const I18N = {
  ...
  en: {
    ...
    tags: {
      text: ["General", "ChatGPT Long", "Claude Legal", "Key Points", "Copywriting", "Short Video", "PPT Outline", "Marketing", "Academic", "Code Docs", "Fiction", "Official"],
      image: ["Midjourney", "DALL-E 3", "Stable Diffusion", "Style", "Colors", "Composition", "E-commerce", "Poster", "Illustration", "Photography", "Architecture", "UI Design"],
      video: ["Pika", "Runway Gen-4", "Kling AI", "Camera", "Style", "Captions", "Script", "Storyboard", "BGM", "AI Edit", "VFX", "Motion"],
      web: ["V0 Site", "Framer", "Components", "Colors", "Interaction", "React", "Responsive", "SEO", "Landing Page", "UI Kit", "Admin", "E-commerce"],
    },
      ...
      <div className="relative z-10">
        ...
        <div className="glass-card rounded-2xl p-5 mb-4">
          ...
          <div className="flex gap-3 mb-4">
            <div className="flex-1"><label htmlFor="file-upload" className="block upload-zone py-5 flex flex-col items-center justify-center min-h-[100px] cursor-pointer"><Upload className="w-5 h-5 text-slate-400 mb-2" /><p className="text-sm text-slate-500">{t.dragOrClick}</p><input type="file" id="file-upload" onChange={handleFileUpload} className="hidden" /></label></div>
          ...
          {[0, 1, 2].map((index) => { const model = cardModels[index]; const prompt = cardPrompts[index]; const modelInfo = MODELS.find((m) => m.key === model); return (
            <div key={index} className="glass-card rounded-xl p-4">
              <div className="relative">
                <button onClick={(e) => { e.stopPropagation(); setOpenDropdown(openDropdown === index ? null : index); }} className="w-full flex items-center justify-between px-3 py-2 text-sm bg-slate-50 rounded-lg hover:bg-slate-100 transition-colors"><span className={model ? "text-slate-700 font-medium" : "text-slate-400"}>{model ? modelInfo?.name : t.selectModel}</span><ChevronDown className="w-4 h-4 text-slate-400" /></button>
                {openDropdown === index && mounted && createPortal(
                  <div onClick={(e) => e.stopPropagation()} className="fixed bg-white border border-slate-200 rounded-lg shadow-2xl max-h-64 overflow-y-auto" style={{ zIndex: 99999, top: '50%', left: '50%', transform: 'translate(-50%, -50%)' }}>
